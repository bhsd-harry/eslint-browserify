...
class ESLint {
  ...
  constructor(options) {
    this.config = options.overrideConfig;
...
describe('comment-directive', () => {
  before(async () => {
    globalThis.eslint ??= (await import('../../../build/eslint.js')).eslint;
    pluginVue = eslint.plugins['vue'] ?? (await import('../../../build/eslint-plugin-vue.js')).default;
