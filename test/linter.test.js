...
describe("Linter with FlatConfigArray", () => {
	...
	describe("Config Options", () => {
		describe("languageOptions", () => {
			...
			describe("parseOptions", () => {
				it.skip("should pass ecmaFeatures to all rules when provided on config", () => {
					const parserOptions = {
						ecmaFeatures: {
							jsx: true,
						},
		...
		describe("rules", () => {
			...
			it.skip("should not be configurable by setting other value", () => {
				const rule = "no-var",
					config = { rules: {} };
	...
	describe("verify()", () => {
		...
		it.skip("should report ignored file when filename isn't matched in the config array", () => {
			const code = "var foo\n    alert('t\\est')";
			const config = {
				rules: {
					"no-var": 1,
		...
		it.skip("should use `cwd` constructor option as config `basePath` when config is not an instance of FlatConfigArray with Posix paths", () => {
			const rule = {
				create(context) {
					return {
						Program(node) {
		...
		it.skip("should use `cwd` constructor option as config `basePath` when config is not an instance of FlatConfigArray with Windows paths", () => {
			const rule = {
				create(context) {
					return {
						Program(node) {
		...
		it.skip("should ignore external files with Posix paths", () => {
		...
		it.skip("should ignore external files with Windows paths", () => {
			const configs = createFlatConfigArray(
				{ files: ["**/*.js"] },
				{ basePath: "C:\\foo" },
			);
		...
		describe("Rule Internals", () => {
			...
			it.skip("should throw an error if a rule with invalid `meta.schema` is enabled in the configuration", () => {
				const config = [
					{
						plugins: {
							test: {
			...
			it.skip("should throw an error if a rule with invalid `meta.schema` is enabled in a configuration comment", () => {
				const config = [
					{
						plugins: {
							test: {
		...
		describe("Rule Severity", () => {
			it.skip("rule should run as warning when set to 1 with a config array", () => {
				const ruleId = "no-var",
		...
		describe("Inline Directives", () => {
			...
			describe("/*eslint*/ Comments", () => {
				...
				describe("when evaluating code with comments to disable rules", () => {
					...
					it.skip("should throw an error when a non-existent rule in config", () => {
						assert.throws(() => {
...
`;
		...
		describe("Error Conditions", () => {
			...
			describe("when using a rule which has been replaced", () => {
				...
				it.skip("should report the new rule", () => {
					assert.throws(() => {
						linter.verify(code, {
	...
	describe("processors", () => {
		...
		describe("preprocessors", () => {
			it.skip("should receive text and filename.", () => {
				const code = "foo bar baz";
			...
			it.skip("should receive text even if a SourceCode object was given.", () => {
				const code = "foo";
				const preprocess = sinon.spy(text => text.split(" "));
			...
			it.skip("should receive text even if a SourceCode object was given (with BOM).", () => {
				const code = "\uFEFFfoo";
				const preprocess = sinon.spy(text => text.split(" "));
				const configs = createFlatConfigArray([extraConfig]);
			...
			it.skip("should catch preprocess error.", () => {
				const code = "foo";
		...
		describe("postprocessors", () => {
			it.skip("should receive result and filename.", () => {
				const code = "foo bar baz";
				const preprocess = sinon.spy(text => text.split(" "));
